...
use crate::entity::entity::{instances_list, make_cube, make_pentagon, Instance, InstanceController, InstanceRaw, MeshBuffer, Vertex, INDICES, INSTANCE_DISPLACEMENT, NUM_INSTANCES_PER_ROW, VERTICES};
...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> State<'a> {
        ...
        let instance_controller = InstanceController::new(instances_list(), 0, vec![make_cube(&device)], &device); 
    ...
    pub fn input(&mut self, event: &WindowEvent) -> bool {
        self.game_loop.process_event(event);
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            render_pass.set_vertex_buffer(1, self.game_loop.instance_controller.instance_buffer.slice(..));
            render_pass.set_pipeline(&self.render_pipeline);
            render_pass.set_bind_group(0, &self.diffuse_bind_group, &[]);
            render_pass.set_bind_group(1, &self.camera_bind_group, &[]);
            // for polygon in &self.entity_buffer {
            let polygon = &self.game_loop.instance_controller.entity_buffers.first().unwrap();
